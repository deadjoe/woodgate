...
from woodgate.core.search import extract_search_results, perform_search, get_product_alerts, get_document_content
from server import initialize_browser, close_browser, handle_cookie_popup
...
async def test_extract_search_results():
    ...
    mock_result1.query_selector.side_effect = lambda selector: {
        ...
        ".search-result-content, .pf-c-card__body": AsyncMock(text_content=AsyncMock(return_value="Summary 1")),
        ".search-result-info span, .pf-c-label": AsyncMock(text_content=AsyncMock(return_value="Solution")),
        ".search-result-info time, .pf-c-label[data-testid='date']": AsyncMock(text_content=AsyncMock(return_value="2023-01-01")),
    }.get(selector, None)
    
    # 模拟第二个结果
    mock_result2.query_selector.side_effect = lambda selector: {
        "h2 a, .pf-c-title a": mock_title2,
        ".search-result-content, .pf-c-card__body": AsyncMock(text_content=AsyncMock(return_value="Summary 2")),
        ".search-result-info span, .pf-c-label": AsyncMock(text_content=AsyncMock(return_value="Article")),
        ".search-result-info time, .pf-c-label[data-testid='date']": AsyncMock(text_content=AsyncMock(return_value="2023-01-02")),
    }.get(selector, None)
...
async def test_perform_search():
    ...
    with patch("woodgate.core.search.handle_cookie_popup") as mock_handle_cookie:
        with patch("woodgate.core.search.extract_search_results", return_value=mock_results) as mock_extract:
            # 调用被测试的函数
...
async def test_get_product_alerts():
    ...
    mock_alert1.query_selector.side_effect = lambda selector: {
        ...
        ".security-severity, .pf-c-label": AsyncMock(text_content=AsyncMock(return_value="Critical")),
        ".portal-advisory-date, time": AsyncMock(text_content=AsyncMock(return_value="2023-01-01")),
        ".portal-advisory-synopsis, .pf-c-card__body": AsyncMock(text_content=AsyncMock(return_value="Alert 1 summary")),
    }.get(selector, None)
    ...
    mock_alert2.query_selector.side_effect = lambda selector: {
        ...
        ".security-severity, .pf-c-label": AsyncMock(text_content=AsyncMock(return_value="Important")),
        ".portal-advisory-date, time": AsyncMock(text_content=AsyncMock(return_value="2023-01-02")),
        ".portal-advisory-synopsis, .pf-c-card__body": AsyncMock(text_content=AsyncMock(return_value="Alert 2 summary")),
    }.get(selector, None)
...
async def test_get_document_content():
    ...
    mock_field.query_selector.side_effect = lambda selector: {
        ".field-label, .pf-c-description-list__term": AsyncMock(text_content=AsyncMock(return_value="Type:")),
        ".field-item, .pf-c-description-list__description": AsyncMock(text_content=AsyncMock(return_value="Solution")),
    }.get(selector, None)
